...
By default, the launcher explicitly limits the official server to the writable `context`, `repos`, `issues`, `pull_requests`, and `users` toolsets. This avoids inheriting additional toolsets that a future server release may add to its own defaults. GitHub App token permissions remain the authorization boundary. Advanced workflows can set these environment variables on the action step:
